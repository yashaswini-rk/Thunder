...
#define FREE(x) HeapFree(GetProcessHeap(), 0, (x))
    ...
    {
        uint16_t result = 0;
        const nlmsghdr* header = reinterpret_cast<const nlmsghdr*>(dataFrame);

        if (NLMSG_OK(header, receivedSize)) {
            const struct ifinfomsg* ifi = reinterpret_cast<const struct ifinfomsg*>(NLMSG_DATA(header));
            ...
            if ((header->nlmsg_type == RTM_NEWLINK) || (header->nlmsg_type == RTM_DELLINK) || (header->nlmsg_type == RTM_GETLINK) || (Type() == RTM_SETLINK)) {
                const IPNetworks::Network& network(networkController[ifi->ifi_index]);

                if (network.IsValid()) {
                    networkController[ifi->ifi_index].Update(reinterpret_cast<const struct rtattr*>(IFLA_RTA(ifi)), receivedSize - sizeof(struct ifinfomsg));
                }
            }

            if (interfaceName.empty() == false) {
                for (INetworkNotification* callback : _callbacks) {
                    callback->Event(interfaceName.c_str());
                }
            } 

            result = header->nlmsg_len;
        } else {
